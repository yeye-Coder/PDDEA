...
class PDDEA:
    ...
    def open_files(self):
        while True:
            ...
            try:
                # Helper function to load and clean data from CSV
                def load_csv(file_path, expected_shape=None):
                    with open(file_path, 'r') as file:
                        ...
                        data = []
                        for line in lines:
                            # Convert string to numbers, handling all delimiters
                    ...
                    if expected_shape:
                        if expected_shape[0] == 1:  # If we expect a single row, we might have multiple rows in the file
                            data_array = data_array.flatten()
                        elif expected_shape[0] > data_array.shape[0]:
                            raise ValueError(f"File should have at least {expected_shape[0]} rows.")
                    
                    return data_array if expected_shape is None else data_array.reshape(expected_shape)

                # Load PRPD and waveform data
                self.prpd = load_csv(prpd_file, expected_shape=(2, -1))  # PRPD should have 2 rows
                self.waveform = load_csv(waveform_file)  # Waveform can vary in shape

                # Validate PRPD and waveform shape relationships
                if self.waveform.shape[0] != self.prpd.shape[1]:
                    messagebox.showerror(
                        "Error",
                        f"Mismatch between PRPD and Waveform data.\n"
                        f"PRPD columns: {self.prpd.shape[1]}, Waveform rows: {self.waveform.shape[0]}."
                    )
                    continue  # Prompt again

                # Debugging: Print shapes
                print(f"PRPD shape: {self.prpd.shape}")
                print(f"Waveform shape: {self.waveform.shape}")
